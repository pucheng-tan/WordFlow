...
TEST_DATE = datetime(2020, 11, 1, tzinfo=timezone.utc)# November 1
...
@pytest.mark.parametrize("path, where_clause, expected_reference", [
    ({"Schools": TEST_SCHOOL_ID, "UserProfiles": TEST_USER_ID, "History": APIService.WHERE_CLAUSE}, [["DateCompleted", ">", TEST_DATE], ["Mode", "==", "0"], ["Duration", "in", [120]]], "2020101513071234"),
    ({"History": APIService.SEARCH_ALL}, [["UserProfile", "==", "BMSoKUzVnRYn103t2Oi6Nacv9X03"], ["AssignmentID", "exists"]], "2020101513091234"),
    ({"Schools": TEST_SCHOOL_ID, "Classrooms": APIService.WHERE_CLAUSE}, [["Members", "array-contains", TEST_USER_ID]], "uWkea95OBZ1d94kaPThK"),
    ({"Schools": TEST_SCHOOL_ID, "Classrooms": TEST_CLASSROOM_ID}, [], TEST_CLASSROOM_ID),
])
def test_get_valid(path, where_clause, json_metadata):
