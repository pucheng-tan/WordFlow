...
@pytest.mark.parametrize("collection, where",
                         [("Schools", [["Name", "==", "X School"]]),
                          ("Schools", [["Name", "exists"]])])
...
@pytest.mark.parametrize("collection, where", [(3, "invalid where clause"),
                                               ("valid_collection_name", 4)])
def test_get_collection_invalid(collection, where, json_metadata):
    json_metadata['id'] = "IT API.2"
    json_metadata[
        'description'] = "invalid queries of wrong data types will raise the expected Exceptions"
...
@pytest.mark.api_call
@pytest.mark.parametrize("collection, where",
                         [("Schools", [["Name", "==", "lkjdsafnlaqk346q9y"]])])
def test_get_collection_no_result(collection, where, json_metadata):
    json_metadata['id'] = "IT API.3"
    json_metadata[
        'description'] = "valid query not pointing to an existing document will return empty result"
...
def test_get_where_clause(json_metadata):
    ...
    json_metadata[
        'description'] = "pass in data to the _get_where_clause function, see if result is as expected without API call"
    pass
...
def test_get_subcollection():
    ...
    pytest.fail(
        "this test hasn't been written yet, so it's marked with xfail. It will be counted towards xpass results in summary, and not actually ran"
    )
    pass
